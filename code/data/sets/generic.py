...
class Dataset(object):    
    ...
    def __init__(self, options):
        ...
        """
        # Directories
        self.dataDirectory = options.get("dataDirectory") or ''
        self.analysisDirectory = options.get("analysisDirectory") or '/tmp'
        self.metadataDirectory = options.get("metadataDirectory") or (self.dataDirectory + '/metadata')
        self.dataPrefix = options.get("dataPrefix") or options.get("dataDirectory") or ''
        ...
        self.verbose = options.get("verbose") or True
        self.forceUpdate = options.get("forceUpdate") or False
        self.checkIntegrity = options.get("checkIntegrity") or True;
        ...
        for line in fileCheck:
            ...
            if line[0] != "#" and len(line) > 1:
                ...
                if (audioPath is not None):
                    ...
                    if (self.checkIntegrity) and (testFileID is None):
                        print('[Dataset][List file] Warning loading task ' + task + ' - File ' + fFileName + ' does not exists ! (Removed from list)')
                    else:
                        if (testFileID):
                            testFileID.close()
    ...
    def flattenData(self, vectorizeResult):
        """ Flatten the entire data matrix """
        size = self.data[0].shape
        prod = np.cumprod(size)
        if (vectorizeResult):
            prod = np.array([prod])
        else:
            prod = np.zeros(size.shape[0] + 1)
            prod[0] = 1; 
            for i in range(size.shape):
                prod[i + 1] = size[i]
        self.data = self.data.reshape(prod) #nn.MapTable(nn.Reshape(prod)).forward(self.data);
